...
<?php
    //var_dump(escapestring($sentence));
...
if ($sentence != "") {
    ...
    foreach ($xml->ma_result->word_list->word as $cur){
        echo escapestring($cur->surface)." | ";
        $arrWord[$cnt] = escapestring($cur->surface);
        $cnt++;
    }

    var_dump(escapestring($cur->surface));
}
?>
</td>
</tr>
...
<?php
    $table = array();
    for ( $i = 0; $i < count($arrWord) - 2; $i++ ) {
        $table[] = array($arrWord[$i], $arrWord[$i + 1], $arrWord[$i + 2]);
    }
    ...
    function buildSentense ( $table_markov )
    {
        ...
        $result = implode('', $key);
        while ( true ) {
            $values = _searchAvailableValues($table_markov, $key);
//    echo("----------------<br>");
//    var_dump($values);
            if(count($values) == 0){
                break;
